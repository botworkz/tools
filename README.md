...
- `botforge pack [--repo-root <dir>] [--compress] [--key <path>] [--template <path>]` — runs a KVM-only Packer build of the VM image natively inside the botforge container, then optionally compresses the qcow2. `--template` selects the Packer template path (file or directory) under the repo root and defaults to `images/` for backwards compatibility. Requires `/dev/kvm`; dependencies and baked images must already be staged beforehand because botforge v1 does not build them. KVM is required; there are no tcg/accelerator options.
