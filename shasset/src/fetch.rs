...
(401, Some(r#"Bearer realm="https://ghcr.io/token",service="ghcr.io",scope="repository:botworkz/svc:pull""#.to_string()), vec![]),
    ...
    const OCI_WWW_AUTH: &str =
        r#"Bearer realm="https://ghcr.io/token",service="ghcr.io",scope="repository:botworkz/svc:pull""#;
    ...
    fn oci_token_exchange_bare_token_uses_x_access_token_basic_auth() {
        ...
        assert_eq!(calls.len(), 1, "expected exactly one basic-auth call to the token endpoint");
        assert_eq!(
            calls[0],
            ("x-access-token".to_string(), "ghs_test_token_no_colon".to_string()),
    ...
    fn oci_token_exchange_no_auth_falls_back_to_anonymous() {
        ...
        assert!(ok, "fetch should succeed");
        assert!(calls.is_empty(), "no basic-auth call expected for anonymous token exchange");
        assert!(anon > 0, "anonymous token exchange should use get() not get_with_basic_auth()");
    }
}
