...
ensure_botwork_sibling() {
  ...
  if [[ ! -f "${BOTWORK_MCP_EXTRA_DIR}/containers/Makefile" ]]; then
    die "botworkz/mcp-extra sibling not found or incomplete at ${BOTWORK_MCP_EXTRA_DIR} (missing containers/Makefile). Clone botworkz/mcp-extra next to this repo or set BOTWORK_MCP_EXTRA_DIR."
  fi
}
