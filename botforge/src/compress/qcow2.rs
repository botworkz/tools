...
pub(crate) fn resize_qcow2_image(disk: &Path, size_str: &str) -> Result<()> {
    ...
    let new_l1_size =
        new_virtual_size.div_ceil(cluster_size).div_ceil(l2_entries_per_table);
    let new_l1_size_u32 =
        u32::try_from(new_l1_size).context("new l1_size does not fit u32")?;
...
mod tests {
    ...
    fn create_overlay_header_fields_are_correct() {
        ...
        assert_ne!(hdr.backing_file_offset, 0, "backing_file_offset must be set");
        ...
        read_exact_at(&mut file, hdr.backing_file_offset, &mut path_buf).expect("read backing path");
    ...
    fn create_overlay_backing_format_extension_is_present() {
        ...
        let ext_magic = read_u64_at(&mut file, QCOW_HEADER_LENGTH_V3 as u64)
            .expect("read extension magic+len");
        ...
        read_exact_at(
            &mut file,
            QCOW_HEADER_LENGTH_V3 as u64 + 8,
            &mut ext_data,
        )
    ...
    fn create_overlay_l1_table_is_all_zeros() {
        ...
        let hdr = read_header(&mut file).expect("read header");
        let l1_entries =
            read_u64_table(&mut file, hdr.l1_table_offset, hdr.l1_size as usize)
                .expect("read l1 table");
    ...
    fn create_overlay_all_clusters_have_positive_refcount() {
        ...
        let rt_entries = read_u64_table(
            ...
            usize::try_from(hdr.refcount_table_clusters).expect("rtc") * (cluster_size as usize / 8),
        )
        .expect("read refcount table");

        for cluster_idx in 0..total_clusters {
            let block_idx = cluster_idx / entries_per_block;
            let entry_in_block = cluster_idx % entries_per_block;
            let rt_entry = rt_entries
                .get(block_idx as usize)
                .copied()
                .unwrap_or(0);
            assert_ne!(rt_entry, 0, "refcount table entry for block {block_idx} must be non-zero");
    ...
    fn resize_no_op_when_same_size() {
        ...
        assert_eq!(before, after, "file must be unchanged when resizing to same size");
    }
    ...
    fn backing_file_size_from_header(file: &mut File) -> usize {
        let mut raw = [0u8; 4];
        read_exact_at(file, HDR_BACKING_FILE_SIZE as u64, &mut raw).expect("read backing_file_size");
