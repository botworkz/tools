...
fn run_semantic_validators(target: SemanticValidationTarget<'_>) -> Result<()> {
    match target {
        SemanticValidationTarget::Build {
            ...
            validate_top_level_uploads("build", uploads)
                .with_context(|| format!("invalid build config: {}", path.display()))?;
        ...
        SemanticValidationTarget::Test {
            ...
            validate_top_level_uploads("test", uploads)
                .with_context(|| format!("invalid test config: {}", path.display()))?;
...
thread_local! {
    ...
    static CLOUD_INIT_SCHEMA_CHECK_OVERRIDE: std::cell::RefCell<Option<fn(&str) -> CloudInitSchemaCheck>> =
        const { std::cell::RefCell::new(None) };
...
mod tests {
    use super::{
        ...
        validate_build_steps, validate_test_ports, validate_test_steps, CloudInitSchemaCheck,
        CloudInitSchemaMode, CompressionType, InputDeclaration, InputType, ReclaimMode, TestConfig,
        TestIso, CLOUD_INIT_SCHEMA_CHECK_OVERRIDE, CLOUD_INIT_SCHEMA_MODE_OVERRIDE,
        CLOUD_INIT_SCHEMA_WARNINGS, MAX_INCLUDE_DEPTH,
    };
...
"#,
    ...
    #[test]
    fn test_load_build_config_still_rejects_invalid_uploads_via_pipeline() {
        let repo = TempDir::new().unwrap();
        write_build_config(
            &repo,
            "build.yaml",
...
"#,
        ...
        assert!(
            ...
            "invalid uploads must still be rejected by loader pipeline: {msg}"
        );
    }
