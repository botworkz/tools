...
"#,
        ...
        assert_eq!(config.image, ImageRef::ShassetDefault("debian-base".to_string()));
...
"#,
        ...
        let config = load_build_config(repo.path(), &repo.path().join("build.yaml")).unwrap();
        assert_eq!(config.image, ImageRef::ShassetDefault("my-base".to_string()));
    ...
    fn test_fragment_rejects_image() {
        ...
        assert!(
            msg.contains("image"),
            "error should mention image: {msg}"
        );
    }
