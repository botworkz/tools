    ...
    for (step_idx, step) in steps.iter().enumerate() {
        ...
        match step {
            ...
            TestStep::Run(run) => {
                    ...
                    StepCondition::Deferred(expr) => {
                        ...
                        resolve_deferred_condition(
                            ...
                            &mut |step_id: &str, output_name: &str| {
                                ...
                                resolve_step_output_reference(run, steps, prior_steps, step_id, output_name)
                            },
                            &mut |output_name: &str| {
                                resolve_fragment_output_reference(run, steps, prior_steps, fragment_output_decls, output_name)
                            },
                        )
                        .unwrap_or(false)
                    }
                ...
                // error otherwise).
                let step_result =
                    resolve_run_step_output_refs(run, steps, step_idx, fragment_output_decls, inherited_step_refs)
                        .and_then(|run_body| {
        ...
        match step {
            TestStep::Run(run) => {
                ...
                let step_result: Result<()> =
                    resolve_run_step_output_refs(run, steps, step_idx, None, &std::collections::BTreeMap::new()).and_then(|run_body| {
...
"#,
    ...
    fn test_resolve_run_refs_backward_invoke_ref_resolves() {
        ...
        let resolved = resolve_run_step_output_refs(&step, &scope, 1, None, &std::collections::BTreeMap::new()).unwrap();
    ...
    fn test_resolve_run_refs_no_refs_is_identity() {
        ...
        let resolved = resolve_run_step_output_refs(&step, &scope, 0, None, &std::collections::BTreeMap::new()).unwrap();
    ...
    fn test_resolve_run_refs_unknown_id_is_error() {
        ...
        let err = resolve_run_step_output_refs(&step, &scope, 0, None, &std::collections::BTreeMap::new()).unwrap_err();
        let msg = format!("{err:#}");
        assert!(
    ...
    fn test_resolver_error_on_secret_output_does_not_leak_raw_value() {
        ...
        let err = resolve_run_step_output_refs(&step, &scope, 1, None, &std::collections::BTreeMap::new()).unwrap_err();
        let msg = format!("{err:#}");
    ...
    fn test_resolve_run_refs_fragment_outputs_ref_resolves_with_default() {
        ...
        let resolved = resolve_run_step_output_refs(&step, &scope, 2, Some(&decls), &std::collections::BTreeMap::new()).unwrap();
    ...
    fn test_resolve_run_refs_fragment_outputs_ref_forward_is_error() {
        ...
        let err = resolve_run_step_output_refs(&step, &scope, 0, Some(&decls), &std::collections::BTreeMap::new()).unwrap_err();
        let msg = format!("{err:#}");
        assert!(
            msg.contains("has not run yet"),
