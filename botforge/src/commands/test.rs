    ...
    for (step_idx, step) in config.steps.iter().enumerate() {
        match step.target {
            ...
            StepTarget::Host => {
                ...
                let suffix = unique_suffix();
                let env_file =
                    std::env::temp_dir().join(format!("botforge-host-env-{suffix}"));
...
fn run_host_step(
    ...
    let mut child = std::process::Command::new(&argv[0])
        ...
        .envs(accumulated_env.iter().map(|(k, v)| (k.as_str(), v.as_str())))
        .spawn()
...
"#,
    ...
    fn test_host_step_injects_accumulated_env() {
        ...
        let _ = std::fs::remove_file(&env_file);
        assert!(result.is_ok(), "accumulated env vars should be visible in host step: {result:?}");
    }
    ...
    fn test_parse_env_file_heredoc_single_line_value() {
        ...
        assert_eq!(
            result,
            vec![("KEY".to_string(), "only line".to_string())]
        );
    }
    ...
    fn test_build_guest_env_preamble_exports_accumulated_vars() {
        ...
        assert!(preamble.contains("export FOO='bar'"), "preamble: {preamble}");
        assert!(
