...
fn run() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Payload(args) => cmd_payload(&cli.config, args),
...
fn cmd_payload(config_path: &Path, args: PayloadArgs) -> Result<()> {
...
mod tests {
    use super::{
        ...
        ssh_command_args, stage_payload_tree, validate_mode_string, validate_payload_install_path,
        validate_relative_staging_path, PayloadConfig, PayloadFile, PayloadImage, PayloadServices,
        SshOptions,
    };
    use std::path::Path;
