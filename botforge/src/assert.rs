...
pub(crate) fn run_assert_services(ssh: &SshOptions, assert_block: &AssertBlock) -> Result<()> {
    ...
    for (name, expectation) in &assert_block.services {
        ...
        print_phase_status("assert", &format!("service {name} exists"), result.exists_ok);
...
fn check_one_service(name: &str, expectation: &AssertService, raw_line: &str) -> ServiceCheckResult {
    ...
    if !expectation.exists {
        ...
        if is_present {
            result
                .failures
                .push(format!("service {name}: expected absent, but the unit exists"));
        }
        return result;
    }
...
mod tests {
    ...
    fn test_check_one_service_absent_expected_present_fails() {
        ...
        assert!(result.failures[0].contains("expected present"), "{result:?}");
    }
