...
mod tests {
    ...
    fn run_round_trip_test(compression_type: CompressionType, cluster_size_bytes: u64) {
        ...
        compress_qcow2_image(&source, &dest, compression_type, &compressor_args, extra_opts)
            .expect("compress");
    ...
    fn native_compress_qemu_img_check_double_compress_zstd_1m() {
        ...
        compress_qcow2_image(&source, &intermediate, CompressionType::Zstd, &args, "-19 -T0")
            .expect("compress A → B");
        compress_qcow2_image(
            &intermediate,
            &final_image,
