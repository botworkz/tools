...
fn write_l1_table(
    ...
    for idx in 0..usize::try_from(l1_size).context("l1_size too large")? {
        let offset = first_l2_offset
            ...
            .context("l2 table offset overflow")?;
        raw[idx * 8..(idx + 1) * 8]
            .copy_from_slice(&(offset | QCOW_OFLAG_COPIED).to_be_bytes());
    }
...
mod tests {
    ...
    fn write_custom_test_image_with_cluster_size(
        ...
        write_exact_at(
            &mut file,
            24,
            &(cluster_size.checked_mul(cluster_count).context("virtual size overflow")?)
                .to_be_bytes(),
        )?;
        write_exact_at(&mut file, 36, &1u32.to_be_bytes())?;
        write_exact_at(&mut file, 40, &cluster_size.to_be_bytes())?;
    ...
    fn run_copied_flag_test(compression_type: CompressionType, cluster_size: u64) {
        ...
        assert_eq!(l1_entries.len(), 1, "mixed fixture should use exactly one L1 entry");
        assert_ne!(l1_entries[0] & QCOW_OFLAG_COPIED, 0, "L1 entry must set COPIED");
        ...
        let l2_entries = read_u64_table(&mut file, l2_offset, l2_entries_per_table).expect("read l2");
        assert_eq!(l2_entries[0], 0, "zero cluster entry should stay unallocated");
