...
mod tests {
    ...
    fn assert_compressed_clusters_sector_disjoint(
        ...
        let l1_entries =
            read_u64_table(file, header.l1_table_offset, l1_size).expect("l1 table");
        ...
        for (l1_idx, &l1_entry) in l1_entries.iter().enumerate() {
            ...
            let l2_offset = aligned_data_offset(l1_entry, cluster_size);
            let l2_table =
                read_u64_table(file, l2_offset, l2_entries_per_table).expect("l2 table");
            for (l2_idx, &l2_entry) in l2_table.iter().enumerate() {
                ...
                let nb_csectors =
                    ((l2_entry >> header.csize_shift()) & header.csize_mask()) + 1;
    ...
    fn write_multi_cluster_test_image(path: &Path, cluster_count: u64) -> Result<()> {
        ...
        write_exact_at(
            &mut file,
            24,
            &(cluster_size * cluster_count).to_be_bytes(),
        )?; // virtual_size
