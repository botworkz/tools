...
fn evaluate_node_runtime(
    ...
    match node {
        ...
        ExprNode::StepOutputReference { step_id, output_name } => {
            (resolver.resolve_step)(step_id, output_name)
        }

        ExprNode::FragmentOutputReference { output_name } => {
            (resolver.resolve_output)(output_name)
        }
        ...
        ExprNode::Not(expr) => {
            Ok(EvaluatedValue::Bool(!evaluate_node_runtime(expr, resolver)?.truthy()))
        }
