...
async fn connect_async(
    ...
    let key_pair = load_secret_key(&ssh.key, None).context("failed to load SSH private key")?;
...
async fn exec_simple_async(
    ...
    match exit_code {
        ...
        None => AttemptResult::Transport(anyhow::anyhow!("SSH channel closed without exit status")),
    }
}
...
async fn exec_logged_async(
    ...
    let effective_connect_timeout = connect_timeout.min(overall_remaining);
...
pub(crate) fn ssh_with_retry(
    ...
    loop {
        match rt.block_on(exec_simple_async(
            ssh,
            remote_command,
            connect_timeout,
            false,
...
mod tests {
    ...
    fn keypair_private_key_round_trips_via_russh() {
        use russh::keys::{load_secret_key, ssh_key::Algorithm};
